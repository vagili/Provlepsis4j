...
try:
    ...
    from .feature import get_projection_props  # type: ignore
    _FEATURES_OK = True
except Exception:
    get_projection_props = None  # type: ignore
    _FEATURES_OK = False
...
def _connect_train_components_via_paths(
    base_g: str,              # kept for symmetry; not used by cypher path now
    ...
    for cid, nodes in groups.items():
        ...
        path_neos = _shortest_path_nodes_on_base_cypher(
            ...
            max_len=20,  # feel free to tune; 10–30 is typical
        )
...
class ExecuteSplitBody(BaseModel):
    ...
    valGraphName: Optional[str] = "valGraph"  # may be None/"" => no VAL created
    ...
    includeEmbeddingProps: bool = False  # include prior embedding props?
...
def execute_master_split(body: ExecuteSplitBody):
    ...
    try:
        ...
        # 2) Tag endpoints and project each split using native gds.graph.project
        # TRAIN
        # TRAIN
        # 1) Tag all ORIGINAL nodes so they appear as nodes in the TRAIN projection
        _tag_all_original_nodes(train_ep_label)

        # 2) Also tag TRAIN endpoints (so even if you later change the WHERE above to include fewer nodes,
        #    endpoints definitely appear)
