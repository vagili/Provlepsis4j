...
import re
from ..db import run_data, current_database  # uses your existing helpers

router = APIRouter()

# If True we prefix "USE <db>\n"; safer is False and pass db=... to driver.
...
# utils/graphify.py (or wherever it lives)
from typing import Any, Dict, List, Tuple

# ---- REPLACE _graphify WITH THIS VERSION ----
...
def _rewrite_shortest_path_cypher(cypher: str) -> str:
    """
    Take the user's query that contains MATCH p = shortestPath(...)
    ...
    This assumes the path variable is named 'p' (which matches your queries).
    """
    # Split off the first RETURN, keep everything before it.
    m = re.search(r"\bRETURN\b", cypher, flags=re.IGNORECASE)
    if m:
        prefix = cypher[:m.start()]
    else:
        # no RETURN, just run as-is and assume it binds 'p'
        prefix = cypher

    # Ensure trailing semicolon doesn't break anything
    prefix = prefix.rstrip().rstrip(";")

    # Rebuild the query to expose nodes/relationships with elementIds
    rewritten = f"""
{prefix}
WITH p
...
def _shortest_path_graph(rows: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    def add_node(n: Dict[str, Any]):
        ...
        # business id from Neo4j property `id`
        node_id_prop = None
        if "id" in props:
        ...
        if nid not in nodes_by_id:
            ...
            # expose the business id under a dedicated field
            if node_id_prop is not None:
                node["nodeId"] = node_id_prop

            # copy remaining properties (don’t overwrite id/labels/nodeId)
    ...
    for row in rows or []:
        ...
        # also handle a wrapped shape like {path: {nodes: [...], relationships: [...]}}
        path = row.get("path")
...
def _graphify(records: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    def add_node(x: Any):
        ...
        if isinstance(x, NeoNode):
            ...
            # two shapes:
            #  - flattened props (many keys at top level)
            #  - wrapped under {"properties": {...}}
            if "properties" in x and isinstance(x["properties"], dict):
                ...
                props = {
                    ...
                    not in (
                        ...
                        "elementId",
                        "element_id",
                        "id",          # <- meta id, we treat separately
                        "segments",
                        "nodes",
                        "relationships",
                    )
        ...
        # business id property (your node property "id" from Neo4j)
        node_id_prop = None
        if isinstance(x, NeoNode):
            ...
            # could be nested under "properties" or flattened
            if "properties" in x and isinstance(x["properties"], dict):
        ...
        if nid not in nodes_by_id:
            ...
            # merge other props (don’t overwrite id/labels/nodeId)
            for k, v in props.items():
                if k not in ("id", "labels", "nodeId"):
                    node_obj[k] = v
    ...
    def add_rel(r: Any, src: Any, trg: Any):
        ...
        if isinstance(r, NeoRel):
            ...
            # e.g. when r is just a string relationship type
            rtype = str(r)
            rid = f"{sid}->{tid}:{rtype}"
            props = {}
    ...
    def consume_value(v: Any):
        ...
        if isinstance(v, NeoRel):
            # relationship alone → we can’t add without ends
            return True
        ...
        if isinstance(v, list):
            # Special case: [ nodeDict, "RELTYPE", nodeDict ]
        ...
        if isinstance(v, dict):
            ...
            # Node-like dicts (id present; labels optional; props may be flattened)
            if node_id_from(v):
                add_node(v)
                # also check if this dict secretly contains a relationship
...
def query_both(body: BothBody):
    ...
    # Keep backward compat: include both "future" and "predicted"
    return {
        "present": present_rows,
        "future": future_rows,              # <-- NEW alias
        "predicted": future_rows,           # existing key
        "databases": {"present": base_db, "future": predicted_db, "predicted": predicted_db},
    }

# ---- NEW graph mode variant ----
class ModeBody(BothBody):
    mode: str = "table"  # "table" | "graph"
...
def query_both2(body: ModeBody):
    ...
    # --- Non-graph mode: behave exactly as before ---
    if body.mode != "graph":
        present_rows = _exec(body.cypher, params, db=base_db)
        future_rows  = _exec(body.cypher, params, db=predicted_db)
        return {
            "present": present_rows,
            "future": future_rows,                    # alias
            "predicted": future_rows,                 # keep old
            "databases": {
                "present": base_db,
                "future": predicted_db,
    ...
    # 1) Special handling for shortestPath(...) queries
    if _contains_shortest_path(body.cypher):
        # Rewrite to a shape that exposes {nodes, relationships}
        sp_cypher = _rewrite_shortest_path_cypher(body.cypher)

        present_rows = _exec(sp_cypher, params, db=base_db)
        future_rows  = _exec(sp_cypher, params, db=predicted_db)

        present_graph = _shortest_path_graph(present_rows)
        future_graph  = _shortest_path_graph(future_rows)
    ...
    # 2) All other queries: use generic _graphify (your existing behaviour)
    present_rows = _exec(body.cypher, params, db=base_db)
    future_rows  = _exec(body.cypher, params, db=predicted_db)

    return {
        "present": _graphify(present_rows),
        "future": _graphify(future_rows),      # alias
        "predicted": _graphify(future_rows),   # keep old
        "databases": {
            "present": base_db,
            "future": predicted_db,
            "predicted": predicted_db,
...
def query_dbs():
    ...
    return {
        ...
        "future": f"{base}-predicted",  # alias for clarity
    }
