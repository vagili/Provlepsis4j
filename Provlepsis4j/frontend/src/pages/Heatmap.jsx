...
// ---------- typography tiers ----------
const FONT_TITLE = { fontSize: 18, fontWeight: 700 };        // big section titles (red)
const FONT_SUBTITLE = { fontSize: 15, fontWeight: 600 };     // embedding names (green)
const FONT_BODY = { fontSize: 14, fontWeight: 400, color: "#111827" }; // everything else (blue)

// tiny color mapper 0..100 → classic white→red heatmap
function cellColor(pct) {
  const t = Math.max(0, Math.min(1, pct / 100)); // clamp 0–1
  const r = 255;
...
export default function Heatmap() {
  ...
  const [metrics, setMetrics] = useState({}); // { FastRP: { train: {...}, ... }, ... }
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setSelected(enabled); // preselect available ones
      } catch {
        setChoices([]);
      }
    })();
  ...
  // convenience: refresh both components
  // const refreshAll = useCallback(() => {
  //   loadEdges();
  //   loadMetrics();
  // }, [loadEdges, loadMetrics]);

  // convenience: refresh both components (edges + metrics)
 ...
 const matrix = useMemo(() => {
  ...
  const hasEmbeddings = baseKeys.length > 0;           // ✅ define this
  ...
  return (
    ...
    >
      ...
      <aside style={{ borderRight: "1px solid #e5e7eb", paddingRight: 12 }}>
        ...
        {/* Refresh button – styled like "Load dataset" */}
        <button
          onClick={refreshAll}
          disabled={loading}
          style={{
      ...
      >
        ...
        {showMetrics && visibleKeys.length > 0 && (
          ...
          >
            ...
            >
              {visibleKeys.map((key) => {
                // const m = metrics[key]?.train || metrics[key];
                ...
                return (
                  ...
                  >
                    <div
                      style={{
                        ...FONT_SUBTITLE, // embedding name size (green)
                        marginBottom: 4,
                      }}
                    >
                      {key}
                      ...
                      {matrix.keys.map((k) => (
                        <th
                          ...
                          style={{
                            padding: "6px 8px",
                            textAlign: "center",
                            ...FONT_SUBTITLE, // column headers size
                          }}
                        >
                          {k}
                        </th>
                        ...
                        {row.map((v, j) => (
                          <td
                            ...
                            style={{
                              ...
                              textAlign: "center",
                              fontWeight: 600,
                              ...FONT_BODY, // cell numbers size + black
                            }}
                          >
                            {pct(v / 100)}
                          </td>
