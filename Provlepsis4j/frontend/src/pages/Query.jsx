...
const HEAT_RED = "rgb(239, 68, 68)";      // #ef4444
const HEAT_YELLOW = "rgb(250, 204, 21)";  // #facc15
const HEAT_GREEN = "rgb(74, 222, 128)";   // #4ade80

function HeatmapLegend({ minPred, maxPred }) {
  const barStyle = {
    background: `linear-gradient(to right, ${HEAT_GREEN} 0%, ${HEAT_YELLOW} 50%, ${HEAT_RED} 100%)`,
  };

  return (
    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
      <span style={{ fontSize: 11, color: "#4b5563" }}>Present</span>
      ...
      <span style={{ fontSize: 11, color: "#4b5563" }}>Future</span>
    </div>
  );
}
...
/* ===================== Main page ===================== */
function getPredictedLevelFromEdge(e) {
  const predRaw =
    e?.predicted ??
    ...
    e?.attrs?.predicted ??
    null;
  if (predRaw === null || predRaw === undefined) return null;
  const n = Number(predRaw);
  return Number.isFinite(n) ? n : null;
}

function computePredRange(graph) {
  const edges = graph?.edges || [];
  let min = Infinity;
  let max = -Infinity;

  for (const e of edges) {
    const lvl = getPredictedLevelFromEdge(e);
    if (lvl != null && lvl > 0) {
      if (lvl < min) min = lvl;
      if (lvl > max) max = lvl;
    }
...
export default function Query() {
  ...
  const [predRange, setPredRange] = useState({ min: null, max: null });
  ...
  // update legend range whenever the future graph changes
  useEffect(() => {
    setPredRange(computePredRange(future.graph));
  }, [future.graph]);
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* right panel  */}
        <div style={{ minWidth: 0, position: "relative", paddingTop: 24 }}>
          {/* Heatmap above predicted graph */}
          <div
            style={{
              position: "absolute",
              top: 0,           
          ...
          >
            <HeatmapLegend minPred={predRange.min} maxPred={predRange.max} />
          </div>
