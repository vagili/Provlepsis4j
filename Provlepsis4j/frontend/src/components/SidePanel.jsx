...
export default function SidePanel({ page, setPage }) {
  ...
  const itNeg = 1;
  const [itRepeats, setItRepeats] = useState(1);
  ...
  async function runIteration() {
    ...
    try {
      // Ensure a positive integer, default to 1 if parsing fails
      const repeats = Math.max(
        1,
        parseInt(itRepeats == null || itRepeats === "" ? "1" : itRepeats, 10)
      );
      ...
      for (let i = 0; i < repeats; i++) {
        ...
        const writtenThis =
          out?.prediction?.relationshipsWritten ??
          out?.relationshipsWritten ??
          out?.added ??
          0;

        totalWritten += Number.isFinite(writtenThis) ? writtenThis : 0;

        // Keep graph + timestamp counter in sync after each iteration
        window.dispatchEvent(new Event("neo4j:db-changed"));
        if (predictedTsDb) {
          await fetchPredictedTimestampCount(predictedTsDb);
        }
      }

      if (lastOut) {
        const targetDb =
          lastOut.targetPredictedDb ||
          lastOut.db ||
          "(unknown-db)";
        const newLevel = lastOut.newPredictedLevel;
        const newTs = lastOut.newTimestamp;

        setItMsg(
          `✓ Ran ${repeats} iteration${
            repeats > 1 ? "s" : ""
          }. Total edges added: ${totalWritten} to ${targetDb} (predicted=${newLevel}, ts=${newTs})`
        );
      } else {
        setItMsg("No iterations executed.");
      }
    } catch (e) {
  ...
  return (
    <aside style={s.panel}>
      ...
      {page === "query" && (
        <section>
          <h3 style={s.h3}>Current Predicted Timesteps</h3>
          <div style={s.section}>
            <div>
              <label style={s.label}>Number of Current Predicted Timesteps</label>
              <input
                type="number"
                readOnly
      ...
      {page === "query" && (
        <section>
          ...
          <div style={s.section}>
            ...
            <div>
              <label style={s.label}>Additional Predicted Timesteps</label>
              <input
                type="number"
                min={1}
                step={1}
                value={itRepeats}
                onChange={(e) => setItRepeats(e.target.value)}
                style={s.number}
              />
            </div>
