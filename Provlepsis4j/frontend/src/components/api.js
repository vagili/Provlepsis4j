...
export async function setNeo4jConfig({ uri, user, password, database }) {
  ...
  // Notify listeners (e.g., Query.jsx) that the active DB changed
  try {
...
export async function uploadGraph(
  ...
  // REQUIRED: field name must be "edges"
  ...
  // NEW: custom dataset name (if provided)
...
export async function executeSplit({
  ...
  // If you already have an Axios instance exported as `api`, prefer this:
...
export async function queryBoth(body) {
  // body can include { cypher, params, futureDb }
...
export async function iteratePrediction(body, opts = {}) {
  // body = { embeddingFamily, embeddingProperty, predictK, candidateMultiplier, probThreshold, negativeRatio }
...
export async function getLpMetrics() {
  ...
  return data; // { base, metrics: { FastRP: {train,...}, ... } }
}
...
export async function getLpGroundTruthEdges() {
  ...
  return res.data; // { base, edges: [ {s,t}, ... ] }
}
...
export async function getPredictedTimestampCount(dbName) {
  ...
  // data = { db, maxPredicted, maxTimestamp }
  return data;
}
